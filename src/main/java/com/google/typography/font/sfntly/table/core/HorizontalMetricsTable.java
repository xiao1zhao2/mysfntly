...
public final class HorizontalMetricsTable extends Table {
	...
	public static class
		 ...
		 * @param glyphId 图形id
		 * @param width   图形宽度
		 * @param data    WritableFontData
		 * @date 2018-02-26
		 */
		public WritableFontData setAdvanceWidth(int glyphId, int width, WritableFontData data) {
			if (glyphId >= this.numHMetrics) {
				glyphId = this.numHMetrics - 1;
			}
			int offset = Offset.hMetricsStart.offset + (glyphId * Offset.hMetricsSize.offset) + Offset.hMetricsAdvanceWidth.offset;
			data.writeUShort(offset, width);
			return data;
		}
		 ...
		 * 后面n个字符未生效
		 * @param glyphId 图形id
		 * @param bearing 图形左侧空余
		 * @param data    WritableFontData
		 * @date 2018-02-26
		 */
		public WritableFontData setLeftSideBearing(int glyphId, int bearing, WritableFontData data) {
			while (glyphId >= this.numHMetrics) {
				glyphId = glyphId - this.numHMetrics;
			}
			int offset = Offset.hMetricsStart.offset + (glyphId * Offset.hMetricsSize.offset) + Offset.hMetricsLeftSideBearing.offset;
			data.writeShort(offset, bearing);
			return data;
		}
	}
}
